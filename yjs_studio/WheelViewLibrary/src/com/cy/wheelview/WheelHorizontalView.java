...
public class WheelHorizontalView extends AbstractWheelView {
	...
	protected void initAttributes(AttributeSet attrs, int defStyle) {
		...
		TypedArray a = getContext().obtainStyledAttributes(attrs,
				R.styleable.WheelHorizontalView, defStyle, 0);
		mSelectionDividerWidth = a.getDimensionPixelSize(
				R.styleable.WheelHorizontalView_selectionDividerWidth,
	...
	public void setSelectorPaintCoeff(float coeff) {
		...
		if (mVisibleItems == 2) {
			...
			shader = new LinearGradient(0, 0, w, 0, colors, positions,
			...
			int[] colors = { c2, c2, c2, c2, 0xff000000, 0xff000000, c2, c2,
					c2, c2 };
	...
	protected void doItemsLayout() {
		mItemsLayout.layout(0, 0, getMeasuredWidth(), getMeasuredHeight() - 2
				* mItemsPadding);
	}
	...
	protected void measureLayout() {
		...
		// XXX: Locating bug
		mItemsLayout.measure(MeasureSpec.makeMeasureSpec(getWidth()
				+ getItemDimension(), MeasureSpec.UNSPECIFIED), MeasureSpec
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (widthMode == MeasureSpec.EXACTLY) {
			...
			width = Math.max(getItemDimension()
	...
	private int calculateLayoutHeight(int heightSize, int mode) {
		mItemsLayout.setLayoutParams(new LayoutParams(
				LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));
		mItemsLayout.measure(MeasureSpec.makeMeasureSpec(0,
		...
		mItemsLayout.measure(
				...
				MeasureSpec.makeMeasureSpec(height - 2 * mItemsPadding,
						MeasureSpec.EXACTLY));
	...
	protected void drawItems(Canvas canvas) {
		...
		int left = (mCurrentItemIdx - mFirstItemIdx) * iw + (iw - getWidth())
				/ 2;
		...
		if (mSelectionDivider != null) {
			//TODO: tangtaotao divider should be shorter,temporary solution
			int margin = dpToPx(20);
			int height = h - margin + dpToPx(5);
			...
			// bug.
			cSeparators.clipRect(leftOfLeftDivider, margin, rightOfLeftDivider, height);
			mSelectionDivider.setBounds(leftOfLeftDivider, margin,
					rightOfLeftDivider, height);
			...
			// On Gingerbread setBounds() is ignored resulting in an ugly visual
			// bug.
			cSeparators.clipRect(leftOfRightDivider, margin, rightOfRightDivider, height);
			mSelectionDivider.setBounds(leftOfRightDivider, margin,
					rightOfRightDivider, height);
			mSelectionDivider.draw(cSeparators);
			cSeparators.restore();
		}
