...
public class ContactHttpClient {
    ...
    public void register(String account, String nickName, String password, final ContactHttpCallback<Void> callback) {
        ...
        NimHttpClient.getInstance().execute(url, headers, bodyString, new NimHttpClient.NimHttpCallback() {
            ...
            public void onResponse(String response, int code, String errorMsg) {
                if (code != 0) {
                    Log.e(TAG, "register failed : code = " + code + ", errorMsg = " + errorMsg);
                    if (callback != null) {
                ...
                try {
                    ...
                    int resCode = resObj.getInt(RESULT_KEY_RES);
    ...
    private String readAppKey() {
        try {
            ApplicationInfo appInfo = AppUtils.getAppContext().getPackageManager()
                    .getApplicationInfo(AppUtils.getAppContext().getPackageName(), PackageManager.GET_META_DATA);
            if (appInfo != null) {
